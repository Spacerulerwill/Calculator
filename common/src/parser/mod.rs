...
impl Parser {
    ...
    fn delete_statement(&mut self) -> Result<Statement, ParserError> {
        ...
        match expr.clone() {
            Expr::Identifier { name } => {
                self.consume_line_delimeter()?;
                Ok(Statement::DeleteVariable(name))
            }
            Expr::Call {
                callee,
                paren: _,
                ...
                match Signature::from_call_expression(*callee, arguments) {
                    Ok((name, signature)) => {
                        Ok(Statement::DeleteFunctionSignature { name, signature })
                    }
                    Err(_) => Err(ParserError::CannotDelete(Box::new(CannotDelete {
                        delete,
                        expr,
                    }))),
                }
            }
            expr => Err(ParserError::CannotDelete(Box::new(CannotDelete {
                delete,
                expr,
            }))),
        }
        ...
        if let Some(token) = self.iter.next() {
            match token.kind {
                TokenKind::Number(num) => {
                    ...
                    Ok(Expr::Number { number: num })
                }
                TokenKind::Identifier(_) => Ok(Expr::Identifier { name: token }),
                TokenKind::LeftParen => {
                    let expr = self.expression()?;
                    self.consume(TokenKind::RightParen)?;
                    Ok(Expr::Grouping {
                ...
                _ => Err(ParserError::ExpectedExpression(Box::new(
                    ExpectedExpression { found: Some(token) },
                ))),
            }
        } else {
            Err(ParserError::ExpectedExpression(Box::new(
